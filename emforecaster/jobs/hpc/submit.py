...
def run_script(virtual_env, modules, cli_input, script_path, cluster="narval"):
    ...
    py_env_activate = os.path.join(
        os.path.expanduser("~"), virtual_env, "bin", "activate"
    )
    ...
    if script_path.endswith(".py"):
        command = (
            f"{load_modules} && {activate_env} && python {script_path} {cli_input}"
        )
...
def submit_job(
    logdir, virtual_env, modules, cli_input, script_path, cluster, /, **kwds
):
    """
    ...
    executor.update_parameters(
        ...
        slurm_account=kwds.get(
            "slurm_account", "def-hinat"
        ),  # Replace with your account
        slurm_mail_user=kwds.get(
            "slurm_mail_user", "xmootoo@gmail.com"
    ...
    # Submit the job
    job = executor.submit(
        run_script, virtual_env, modules, cli_input, script_path, cluster
    )
...
def load_configs(exp_name):
    ...
    with open(
        os.path.join("./emforecaster/jobs/hpc", exp_name, "compute_canada.yaml")
    ) as f:
        cc_config = yaml.safe_load(f)

    # SLURM Configuration
    with open(os.path.join("./emforecaster/jobs/hpc", exp_name, "slurm.yaml")) as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="test",
        help="Experiment name dir from emforecaster/jobs/exp",
    )
