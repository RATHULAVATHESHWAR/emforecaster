...
from emforecaster.layers.timesnet.timesblock import TimesBlock
from emforecaster.layers.timesnet.embed import DataEmbedding
...
class TimesNet(nn.Module):
    ...
    def __init__(
        ...
        self.enc_embedding = DataEmbedding(
            c_in=num_channels, d_model=d_model, dropout=dropout, max_len=max_len
        )
    ...
    def forecast(self, x):
        ...
        # embedding
        x = self.enc_embedding(
            x.permute(0, 2, 1)
        )  # (batch_size, seq_len, num_channels)
        x = self.predict_linear(x.permute(0, 2, 1)).permute(
            0, 2, 1
        ...
        if self.revout:
            out = self.revin(
                out.permute(0, 2, 1), mode="denorm"
            )  # (batch_size, num_channels, pred_len)
