...
from emforecaster.layers.dlinear.series_decomp import series_decomp


class DLinear(nn.Module):
    """
    Paper link: https://arxiv.org/pdf/2205.13504.pdf
    Taken from: https://github.com/thuml/emforecaster-Library/blob/main/models/DLinear.py
    """
    ...
    def __init__(
        ...
        if self.task == "classification":
            self.projection = nn.Linear(num_channels * seq_len, num_classes)
    ...
    def forward(self, x_enc):
        if self.task == "forecasting":
            ...
            return dec_out[:, -self.pred_len :, :].permute(
                0, 2, 1
            )  # (batch_size, num_channels, pred_len)
        if self.task == "classification":
            dec_out = self.classification(
                x_enc
