...
# Utils
from emforecaster.utils.utils import Transpose, Unsqueeze
...
class EMForecaster(nn.Module):
    def __init__(
        ...
        if pos_enc == "learnable":
            self.pos_enc = nn.Parameter(
                torch.randn(1, 1, self.num_patches * patch_embed_dim)
            )
        elif pos_enc == "none":
            self.pos_enc = 0
    ...
    def forward(self, x):
        ...
        if self.independent_patching:
            ...
            x = self.patch_model(x).view(
                B, self.num_patches, self.patch_embed_dim
            )  # (B, num_patches, patch_embed_dim)
        else:
        ...
        if self.return_head:
            ...
            # Flatten
            x = x.view(
                B, 1, self.num_patches * self.patch_embed_dim
    ...
    def get_patch_model_config(self):
        map = {"DLinear": self.args.dlinear, "TSMixer": self.args.tsmixer}

        return map[self.patch_model_id]

    def _init_revin(self, revout: bool, revin_affine: bool) -> None:
        self._revin = True
        self.revout = revout
        self.revin_affine = revin_affine
    ...
    def get_patch_model(self) -> nn.Module:
        if self.independent_patching:
            ...
            num_channels = (
                self.num_patches
            )  # Processes (num_patches, patch_dim) entirely (patch-dependent)
            individual = False
