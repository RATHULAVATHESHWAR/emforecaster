...
def compute_channel_accuracy(logits, target, ch_ids, u, args, calibrator=None):
    ...
    # Results
    preds = torch.stack(preds).view(
        -1
    )  # Shape (num_unique_ch,) if num_classes = 2 and (num_unique_ch, num_classes) if num_classes > 2
...
def get_metrics(
    ...
    if mode == "window":
        if args.exp.batchwise_metrics:
            assert all(
                isinstance(x, list) for x in [logits, target]
    ...
    # Channel metrics
        ...
        if args.exp.batchwise_metrics:
            assert all(
                isinstance(x, list) for x in [logits, target, ch_ids, u]
            ...
            for i in range(num_batches):
                metrics = compute_channel_accuracy(
