...
from emforecaster.config.config import Global
from emforecaster.utils.classification import sync, gather_tensor
...
class CalibrationModel:
    ...
    def compile_predictions(self, logits, targets, ch_ids=None):
        ...
        if self.args.open_neuro.ch_loss_type == "BCE":
            ...
            raise ValueError(
                f"Invalid loss_type: {self.args.open_neuro.ch_loss_type}. Use 'BCE' or 'CE'."
            )
        ...
        if self.calibration_model == "isotonic_regression":
            if self.args.data.num_classes > 2:
                calibrated_probs = np.column_stack(
                    [
                        model.predict(probs_np[:, i])
                        for i, model in enumerate(self.sklearn_model)
                    ]
                )
            ...
            calibrated_probs = self.sklearn_model.predict_proba(
                p_cal=self.p_cal, y_cal=self.y_cal, p_test=p_test
            )
        ...
        if calibration_model == "isotonic_regression":
            if self.args.data.num_classes == 2:
                ...
                return [
                    ...
                    for _ in range(self.args.data.num_classes)
                ]
            ...
            base_clf = RandomForestClassifier(
                n_estimators=100, random_state=self.args.exp.seed
            )
            return CalibratedClassifierCV(estimator=base_clf, method="isotonic", cv=5)
        else:
            raise NotImplementedError(
                f"Calibration model '{calibration_model}' not implemented."
            )
...
class BetaCalibration:
    ...
    def fit(self, probs, labels):
        def beta_nll(params):
            return -np.sum(
                labels * np.log(self._calibrate_probs(probs, params[0], params[1]))
                + (1 - labels)
                * np.log(1 - self._calibrate_probs(probs, params[0], params[1]))
            )
...
class TestCalibrationModel(unittest.TestCase):
    def test_calibration_methods(self):
        ...
        for method in methods:
            with self.subTest(method=method):
                ...
                window_probs, targets, _ = calibrator.compile_predictions(
                    logits, labels
                )
                ...
                self.assertTrue(
                    torch.all(calibrated_probs >= 0)
                    and torch.all(calibrated_probs <= 1)
                )
    ...
    def test_multiclass_calibration(self):
        ...
        for method in ["isotonic_regression", "venn_abers"]:
            ...
            self.assertTrue(
                torch.all(calibrated_probs >= 0) and torch.all(calibrated_probs <= 1)
            )
            self.assertTrue(
                torch.allclose(
                    calibrated_probs.sum(dim=1),
                    torch.ones(n_samples).to(device),
                    atol=1e-6,
                )
            )
