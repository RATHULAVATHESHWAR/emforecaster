...
def get_dataloaders(
    ...
    if sample_sizes and all(
        size is not None for size in sample_sizes
    ):  # If every entry in sample_sizes is not None

        # Convert to number of samples from proportions
        if all(0 <= size < 1 for size in sample_sizes):
            num_samples = [
                int(len(datasets[i]) * sample_sizes[i]) for i in range(len(datasets))
            ]
        else:
        ...
        indices = [
            list(
                RandomSampler(
                    datasets[i], replacement=False, num_samples=num_samples[i]
                )
            )
    ...
    for sample in samples:
        dataloader = DataLoader(
            sample, batch_size=batch_size, num_workers=num_workers, shuffle=shuffle
        )
...
class DatasetLTSF(Dataset):
    ...
    def __init__(
        ...
        match flag:
            ...
            case _:
                raise ValueError(
                    f"type {flag} does not exist. Only types are (train, test, & val)"
                )
    ...
    def rd(self):
        ...
        if not os.path.isdir(self.root_path):
            raise IOError(
                f"root directory {self.root_path} does not exist, please try one that exist..."
            )
        ...
        assert file.endswith(
            ".csv"
        ), f"{self.__class__.__name__} only supports csv files formats..."

        if not os.path.isfile(file):
            _suggested = [f for f in os.listdir(self.root_path) if f.endswith(".csv")]
            _suggested_stdout = (
                f'please try ({", ".join(_suggested)})'
                if len(_suggested) != 0
                else "No other csv files exist in this folder"
            )
            raise FileNotFoundError(
                f"""{file} does not exist within, {_suggested_stdout}"""
            )

        self.scaler = StandardScaler()
        df_raw = pd.read_csv(os.path.join(self.root_path, self.dataset_file))
        ...
        if self.features == "all":
            ...
            ), "can not select a target, and not choose a target"
            assert (
                df_raw.columns[1:].isin([self.target]).any()
            ), f"{self.target} is not in the column={df_raw.columns[1:]}"
    ...
    def __getitem__(self, idx):
        ...
        if self.learning_type == "sl":
            output = (
                (seq_x.T, seq_y.T, seq_x_steps, seq_y_steps)
                if self.use_time_features
                else (seq_x, seq_y)
            )
        else:
            output = (seq_x.T, seq_x_steps) if self.use_time_features else (seq_x)
    ...
    def __str__(self):
        return (
            f'DatasetLTSF(root_path="{self.root_path}", '
