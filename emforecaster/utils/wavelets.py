...
def wavelet_features(
    ...
    with torch.no_grad():
        ...
        frequencies = torch.tensor(frequencies)[
            idx
        ]  # Reorder frequencies to match scales

        # Perform continuous wavelet transform using ptwt
        wavelet_object = pywt.ContinuousWavelet(wavelet)

        try:
            coefficients, _ = ptwt.continuous_transform.cwt(
                signal.unsqueeze(0), scales, wavelet_object
            )
            ...
            print(
                f"Min frequency: {frequencies.min().item()}, Max frequency: {frequencies.max().item()}"
            )
            raise
...
def plot_signal_and_scalogram(
    time,
    signal,
    coefficients,
    frequencies,
    title="Signal and Wavelet Transform",
    save_scalogram=False,
    interpolate=False,
):
...
if __name__ == "__main__":
    ...
    f_max = 300
    wavelet = "morl"
    transformed_coeffs, frequencies = wavelet_features(
        signal, fs, num_scales, f_min, f_max, wavelet
    )
