...
from emforecaster.config.config import Global, load_config
from emforecaster.jobs.hpc.submit import submit_job, load_configs
...
def run_grid_search(job_name: str, cc: bool = False, acc: str = "None"):
    ...
    base_path = os.path.join(project_root, "emforecaster", "jobs", "exp")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "ablation", type=str, default="sss", help="Name of the ablation"
    )
    parser.add_argument(
        "--cc",
        action="store_true",
        help="Uses Compute Canada and instead submits an ablation job.",
    )
