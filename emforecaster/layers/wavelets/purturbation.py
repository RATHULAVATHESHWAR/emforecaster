...
def inverse_wavelet_transform_series(data, original_shape, wavelet="db4", level=1):
    ...
    for i in range(original_shape[1]):
        ...
        coeffs_len = [
            len(pywt.wavedec(np.zeros(original_shape[0]), wavelet, level=level)[j])
...
def modify_wavelet_coeffs(data, modification_factor=0.3, additional_noise_factor=0.5):
    ...
    base_noise = np.random.normal(
        0, np.std(data, axis=0) * modification_factor, data.shape
    )

    # Additional noise
    extra_noise = np.random.normal(
        0, np.std(data, axis=0) * additional_noise_factor, data.shape
    )
...
def wavelet_predictions(
    model, window, num_samples=100, wavelet="db4", level=1, modification_factor=0.05
):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for _ in range(num_samples):
            ...
            # Inverse transform
            reconstructed_data = inverse_wavelet_transform_series(
                modified_data, original_data.shape, wavelet, level
            )
